...
TARGET = "PL_MEM.SCRATCH.WORD_01"
# TARGET = "F1_IPBUS.payload.block_ram3.MEM"
COMMAND = 5
BLOCK_SIZES = [2**i for i in range(8, 19)]
...
def main():
    with open(f'{target_underscore}.log','w+') as f:
        f.write(f"Block_Size, Speed, loops, Transfer Size\n")
        command = f"./test_stand -c {COMMAND} -n {TARGET} --block_size 4 -l 1000 -w true"
        ...
        for block_size in BLOCK_SIZES:
            ...
            command = f"./test_stand -c {COMMAND} -n {TARGET} --block_size {block_size} -l {loop_numbers[block_size]} > output_{target_underscore}.txt"
            subprocess.run(command, shell=True)
            log = open(f"output_{target_underscore}.txt", "r")
            output = log.readlines()
            log.close()
